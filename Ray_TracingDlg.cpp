...
BOOL SAPicture_only_exit_one_flag = 0;
OptSys sys;
double* nds = nullptr;
double* ncs = nullptr;
double l = 0;
...
//	ON_WM_CTLCOLOR()
	...
	ON_COMMAND(ID_DRAW_SA_CURVE, &CRayTracingDlg::OnDrawSaCurve)
	ON_COMMAND(ID_Draw_2D, &CRayTracingDlg::OnDraw2d)
	ON_WM_HELPINFO()
//	ON_WM_ERASEBKGND()
	ON_COMMAND(ID_USER_INSTRUCTION, &CRayTracingDlg::OnUserInstruction)
	ON_BN_CLICKED(IDC_MFCBUTTON1, &CRayTracingDlg::OnBnClickedMfcbutton1)
END_MESSAGE_MAP()
...
BOOL CRayTracingDlg::OnInitDialog()
{
	...
	HICON hicon_btn = NULL;
	switch (bgcmode)
	{
		case 0:
			hicon_btn = (HICON)LoadImage(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDI_ICON1), IMAGE_ICON, width - 20, height - 20, LR_DEFAULTCOLOR | LR_CREATEDIBSECTION);
			break;
		case 1:
			hicon_btn = (HICON)LoadImage(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDI_ICON3), IMAGE_ICON, width - 20, height - 20, LR_DEFAULTCOLOR | LR_CREATEDIBSECTION);
			break;
		case 2:
			hicon_btn = (HICON)LoadImage(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDI_ICON4), IMAGE_ICON, width - 20, height - 20, LR_DEFAULTCOLOR | LR_CREATEDIBSECTION);
			break;
		default:
			hicon_btn = (HICON)LoadImage(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDI_ICON5), IMAGE_ICON, width - 20, height - 20, LR_DEFAULTCOLOR | LR_CREATEDIBSECTION);
			break;
	}
	m_Button_Cal.SetIcon(hicon_btn);*/

	//ButtonTransparent m_Btn;
	//GetDlgItem(IDC_BUTTON_CALCULATE)->ModifyStyle(0, BS_OWNERDRAW, 0); //设定为自绘
	//m_Btn.Attach();

	//CMFCButton* pMfcButton = (CMFCButton*)GetDlgItem(IDC_MFCBUTTON1);
	//CRect rect;
	//m_mfcbutton.GetWindowRect(rect);
	//long width = rect.right - rect.left;
	//long height = rect.bottom - rect.top;
	//HICON hicon_btn = (HICON)LoadImage(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDI_ICON1), IMAGE_ICON, width - 20, height - 20, LR_DEFAULTCOLOR | LR_CREATEDIBSECTION);
	//m_mfcbutton.SetIcon(hicon_btn);
	//HBITMAP hBmp, hBmpHot;
	//m_Bg1.GetHBITMAP(NULL, &hBmp);
	//m_Bg2.GetHBITMAP(NULL, &hBmpHot);
	//pMfcButton->SetImage(hBmp, TRUE, hBmpHot);
	/*pMfcButton->SetIcon(hicon_btn);*/
...
void CRayTracingDlg::OnPaint()
{
	...
	else
	{
		...
		{
		case 0:
			hicon_btn = (HICON)LoadImage(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDI_ICON6), IMAGE_ICON, width - intv, height - intv, LR_DEFAULTCOLOR | LR_CREATEDIBSECTION);
			break;
		case 1:
			hicon_btn = (HICON)LoadImage(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDI_ICON9), IMAGE_ICON, width - intv, height - intv, LR_DEFAULTCOLOR | LR_CREATEDIBSECTION);
			break;
		case 2:
			hicon_btn = (HICON)LoadImage(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDI_ICON2), IMAGE_ICON, width - intv, height - intv, LR_DEFAULTCOLOR | LR_CREATEDIBSECTION);
			break;
		case 3:
			hicon_btn = (HICON)LoadImage(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDI_ICON8), IMAGE_ICON, width - intv, height - intv, LR_DEFAULTCOLOR | LR_CREATEDIBSECTION);
			break;
		case 4:
			hicon_btn = (HICON)LoadImage(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDI_ICON7), IMAGE_ICON, width - intv, height - intv, LR_DEFAULTCOLOR | LR_CREATEDIBSECTION);
			break;
		default:
			hicon_btn = (HICON)LoadImage(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDI_ICON6), IMAGE_ICON, width - intv, height - intv, LR_DEFAULTCOLOR | LR_CREATEDIBSECTION);
			break;
...
void CRayTracingDlg::OnSaveResult()
{
	...
	if(nltem != 0)
	{
		for (v=0;v<nltem;v++)
			...
			{
				if (j != 1)
					outFile << CT2A(m_ACCESSList2.GetItemText(v, j).GetString()) << ',';
				else
					outFile << CT2A(m_ACCESSList2.GetItemText(v, j).GetString()) << endl;
			}	
	}
	outFile.close();
	// TODO: 在此添加命令处理程序代码
...
HBRUSH CRayTracingDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
	...
	if (pWnd->GetDlgCtrlID() == IDC_BUTTON_CALCULATE)
	{
		pDC->SetBkMode(TRANSPARENT);
		return (HBRUSH)GetStockObject(NULL_BRUSH);
	}
...
void CRayTracingDlg::OnCalculateResult()
{
	...
	if (ObjectFactorStr[0] == "无限")
	{
		...
		y_or_W = TransStrToDouble(ObjectFactorStr[3]);
		sys.set_sys(a, nsf, dists, rs, nds);
		sys.cal_allres(res, l, y_or_W, nfs, ncs);
		//系统里存了a\nfs\dists\rss
		//全局变量 l,y_orW,nfs,ncs
		//	cal_res(sys,)
		//double *tmp=sys.get_SAs(l);
		//double* tmp = sys.get_DTs(l, y);
		//double* tmp = sys.get_LCAys()
	}
	else
	{
		l = TransStrToDouble(ObjectFactorStr[0]);
		y_or_W = TransStrToDouble(ObjectFactorStr[1]);
		a = TransStrToDouble(ObjectFactorStr[2]);
		sys.set_sys(a, nsf, dists, rs, nds);
		sys.cal_allres(res, l, y_or_W, nfs, ncs);
	}
	...
	string* ParameterName = new string[ressize];
	for (v = 0; v < ressize; v++)
	{
		switch (v)
		{
			case 0:
				ParameterName[v] = "系统焦距 f'";
				break;
			case 1:
				ParameterName[v] = "像方主面位置 lH'";
				break;
			case 2:
				ParameterName[v] = "出瞳距 lp'";
				break;
			case 3:
				ParameterName[v] = "理想像距 ld0'";
				break;
			case 4:
				ParameterName[v] = "全孔径实际像距 ld'";
				break;
			case 5:
				ParameterName[v] = "0.7孔径实际像距 lud'";
				break;
			case 6:
				ParameterName[v] = "全视场理想像高 yd0'";
				break;
			case 7:
				ParameterName[v] = "0.7视场理想像高 ywd0'";
				break;
			case 8:
				ParameterName[v] = "全视场实际像高 yd'";
				break;
			case 9:
				ParameterName[v] = "0.7视场实际像高 ywd'";
				break;
			case 10:
				ParameterName[v] = "理想像距 lf0'";
				break;
			case 11:
				ParameterName[v] = "全孔径实际像距 lf'";
				break;
			case 12:
				ParameterName[v] = "0.7孔径实际像距 luf'";
				break;
			case 13:
				ParameterName[v] = "理想像高 yf0'";
				break;
			case 14:
				ParameterName[v] = "全视场实际像高 yf'";
				break;
			case 15:
				ParameterName[v] = "0.7视场实际像高 ywf'";
				break;
			case 16:
				ParameterName[v] = "理想像距 lc0'";
				break;
			case 17:
				ParameterName[v] = "全孔径实际像距 lc'";
				break;
			case 18:
				ParameterName[v] = "0.7孔径实际像距 luc'";
				break;
			case 19:
				ParameterName[v] = "理想像高 yc0'";
				break;
			case 20:
				ParameterName[v] = "全视场实际像高 yc'";
				break;
			case 21:
				ParameterName[v] = "0.7视场实际像高 ywc'";
				break;
			case 22:
				ParameterName[v] = "全孔径球差 SA";
				break;
			case 23:
				ParameterName[v] = "0.7孔径球差 SAu";
				break;
			case 24:
				ParameterName[v] = "全孔径位置色差 LCAx";
				break;
			case 25:
				ParameterName[v] = "0.7孔径位置色差 LCAxu";
				break;
			case 26:
				ParameterName[v] = "0孔径位置色差 LCAxu0";
				break;
			case 27:
				ParameterName[v] = "全视场倍率色差 LCAy";
				break;
			case 28:
				ParameterName[v] = "0.7视场倍率色差 LCAyw";
				break;
			case 29:
				ParameterName[v] = "初级子午场曲 xt'";
				break;
			case 30:
				ParameterName[v] = "初级弧矢场曲 xs'";
				break;
			case 31:
				ParameterName[v] = "初级像散 xts'";
				break;
			case 32:
				ParameterName[v] = "全视场绝对畸变 adt";
				break;
			case 33:
				ParameterName[v] = "0.7视场绝对畸变 adtw";
				break;
			case 34:
				ParameterName[v] = "全视场相对畸变 rdt";
				break;
			case 35:
				ParameterName[v] = "0.7视场相对畸变 rdtw";
				break;
			case 36:
				ParameterName[v] = "全市场全孔径子午彗差coma1";
				break;
			case 37:
				ParameterName[v] = "全视场0.7孔径子午彗差coma2";
				break;
			case 38:
				ParameterName[v] = "0.7视场全孔径子午彗差coma3";
				break;
			case 39:
				ParameterName[v] = "0.7视场0.7孔径子午彗差coma4";
				break;
		}
	}
	CreateList2();
	...
	for (v = 0; v < ressize; v++)
		for (j = 0; j < 2; j++)
		{
			...
			if (j == 0)
			{
				_bstr_t bstr(ParameterName[v].c_str());
				lvitemAdd.pszText = (LPTSTR)bstr;
				m_ACCESSList2.InsertItem(&lvitemAdd);//插入行
			}
			else
			{
				_bstr_t bstr(DoubleToString(res[v],8).c_str());
				lvitemAdd.pszText = (LPTSTR)bstr;
			}
			m_ACCESSList2.SetItem(&lvitemAdd);
		}
...
void CRayTracingDlg::ListCorrectZero(CListCtrl& m_List)
{
	...
	{
		int v;
		int size = ErrorPlacePermanent[0];   //第0个元素存放数组大小  之后每两个存放行列坐标
		int nlsub = m_List.GetHeaderCtrl()->GetItemCount();
		if (nlsub != 6)
		{
			for (v = 0; v < size / 2; v++)
			{
				if ((ErrorPlacePermanent[2 * v + 1] + ErrorPlacePermanent[2 * v + 2] == 0) && nlsub == 4)
...
string CRayTracingDlg::DoubleToString(const double value, unsigned int precisionAfterPoint = 8)
{
	ostringstream out;
	// 清除默认精度
...
void CRayTracingDlg::OnDrawSaCurve()
{
	// TODO: 在此添加命令处理程序代码
	SAPicture* Dlg = NULL;
	if (SAPicture_only_exit_one_flag == 0)
	{
		//判定对话框是否有所指向
		Dlg = new SAPicture();
		Dlg->Create(IDD_SA_PICTURE, this);//创建
		Dlg->ShowWindow(SW_SHOW);//显示
		Dlg->CenterWindow();
		SAPicture_only_exit_one_flag = 1;
	}
	else
		AfxMessageBox(L"请勿重复创建对话框！");
}


void CRayTracingDlg::OnDraw2d()
{
	// TODO: 在此添加命令处理程序代码
	int nsf = sys.get_nsf();
	int k = 0;
	double rho = sys.sf[k].get_rho();
	double d = sys.sf[k].get_d();
}


BOOL CRayTracingDlg::OnHelpInfo(HELPINFO* pHelpInfo)
{
...
//BOOL CRayTracingDlg::OnEraseBkgnd(CDC* pDC)
//{
//	// TODO: 在此添加消息处理程序代码和/或调用默认值
//	BOOL* flag;
//	OnUserInstruction(flag);
//	return *flag;
//}


void CRayTracingDlg::OnUserInstruction()
{
	/*::WinExec("C:\Program Files (x86)\Adobe\Acrobat DC\Acrobat Ray Tracing帮助文档.pdf", SW_SHOW);*/
	int judge = (int)ShellExecuteA(NULL, "open", "Ray Tracing帮助文档.pdf", NULL, NULL, SW_SHOWNORMAL);
	if (judge > 32)
		help_flag = 1;
	else
	{
		AfxMessageBox(L"找不到帮助文档！\n请勿修改帮助文档名称！\n 可查看帮助文档与应用程序是否置于相同路径下！");
		help_flag = 0;
	}
	// TODO: 在此添加命令处理程序代码
}
...
void CRayTracingDlg::OnBnClickedMfcbutton1()
{
	// TODO: 在此添加控件通知处理程序代码
